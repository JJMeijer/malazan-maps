...
(() => {
  ...
  var placeMarker = (mapId) => {
    ...
    const markerRelativeX = Math.round(
      parseInt(markerx) * mapRealDimensions.width / mapNaturalDimensions.naturalWidth - markerRealDimensions.width / 2
    ) + leftPadding;
    const markerRelativeY = Math.round(
      parseInt(markery) * mapRealDimensions.height / mapNaturalDimensions.naturalHeight - markerRealDimensions.height
    ) + topPadding;
    ...
    imageWrappers.forEach((imageWrapper) => {
      ...
      imageWrapper.onwheel = (event) => {
        ...
        if (delta > 0) {
          scale = clamp(
            scale * scaleFactor,
            1 / __pow(scaleFactor, maxWheelUp),
            __pow(scaleFactor, maxWheelDown)
          );
        } else {
          scale = clamp(
            scale / scaleFactor,
